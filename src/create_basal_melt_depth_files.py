...
OptimESM
- for use with custom subroutine `fwf_bm` in sbcfwf.F90
- output files: `basal_melt_depth{1/2}.nc` with variable `bmdepth`
...
PRIMAVERA
- uses the isf module, based on Mathiot2017 distribution,
  files interpolated from eORCa to ORCA grids on local machine with xESMF because on the hpc20920
  machine there are package conflicts
- both ORA1 and ORCA025
- output file: `` with variables
    - ``
    - ``

SOFIAMIP
- uses the isf module
- circum-Antarctic full depth
...
def basal_melt_depths_OptimESM():
    ...
    for i in [1,2]:
        fn_out = f'{output_path}/OptimESM/basal_melt_depth{i}.nc'
...
def basal_melt_depths_PRIMAVERA():
    """
    basal_melt_depths.nc: depth_bot, depth_top
    """

    return


def basal_melt_depths_SOFIAMIP():
    """
    """
...
if __name__=="__main__":
    # OptimeESM
    test_file_existence(fn_do)
    test_file_existence(fn_fwf)
